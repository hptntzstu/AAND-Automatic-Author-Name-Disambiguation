...
def get_tfidf(text: Optional[str], counts: list) -> dict:
    if text is None:
        return dict()    
    ...
    return dict((i, counter[i] / len (text) * math.log(D / cdict[i])) for i in counter)

def get_tfidf_cosine(text_a: Optional[str], text_b: Optional[str], counts: list) -> float: 
    if text_a is None or text_b is None:
        return NUMPY_NAN

    a = get_tfidf(text_a,counts)
    b = get_tfidf(text_b,counts)

    np1 = dict.fromkeys(a.keys() | b.keys(), 0)
    np2 = np1.copy()
    np1.update(a)
    np2.update(b)
    np1 = np.array(list(np1.values()))
    np2 = np.array(list(np2.values()))
    
    return cosine_sim(np1, np2) + 1
...
def get_text_ngrams_words(text: Optional[str], stopwords: Set[str] = STOPWORDS) -> Counter:
    ...
    # {'all':ngrams, 'uni':unigrams, 'bi':Counter(bigrams), 'tri':Counter(trigrams)}
    return ngrams, text_split
...
def counter_dice(
    ...
    """
    Computes dice overlap between two Counters
    ...
    intersection_sum = 2 * sum((counter_1 & counter_2).values())
    union_sum = sum(counter_1.values()) + sum(counter_2.values()) 
    score = intersection_sum / min(union_sum, denominator_max)
    return min(score, 1)
...
def dice(
    ...
    """
    Computes dice overlap between two sets
    ...
    Returns
    -------
    float: the dice overlap
    """
    if len(set_1) == 0 or len(set_2) == 0:
        return default_val

    score = 2 * len(set_1.intersection(set_2)) / (len(set_1) + len(set_2))
    return score
