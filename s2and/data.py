...
class Signature(NamedTuple):
    ...
    author_info_affiliations_ngrams_words: Optional[dict]
    author_info_affiliations_word_split: Optional[List[str]]
    author_info_affiliations_ngrams_chars: Optional[dict]
    author_info_coauthor_ngrams_chars: Optional[Counter]
    author_info_coauthor_ngrams_words: Optional[Counter]
...
class Paper(NamedTuple):
    ...
    title_word_split: Optional[List[str]]
    authors: List[Author]
    venue: Optional[str]
    journal_name: Optional[str]
    title_ngrams_chars: Optional[Counter]
    venue_ngrams: Optional[Counter]
    venue_ngrams_words: Optional[Counter]
    venue_word_split: Optional[List[str]]
    journal_ngrams: Optional[Counter]
    journal_ngrams_words: Optional[Counter]
    journal_word_split: Optional[List[str]]
...
class ANDData:
    ...
    def __init__(
        ...
        W2Vmodels: Optional[Union[str, Dict]] = None,
        idf_counts : Optional[Union[str, Dict]] = None,
    ):
        if mode == "train":
        ...
        for paper_id, paper in self.papers.items():
            self.papers[paper_id] = Paper(
                ...
                venue_ngrams=None,
                venue_ngrams_words=None,
                venue_word_split=None,
                journal_ngrams=None,
                journal_ngrams_words=None,
                journal_word_split=None,
        ...
        for signature_id, signature in self.signatures.items():
            self.signatures[signature_id] = Signature(
                ...
                author_info_affiliations_ngrams_words=None,
                author_info_affiliations_word_split=None,
                author_info_affiliations_ngrams_chars=None,
                author_info_coauthor_ngrams_chars=None,
                author_info_coauthor_ngrams_words=None,
        ...
        logger.info("preprocessing papers") #add pre calculate TF_IDF
        ...
        logger.info("loading W2Vmodels")
        self.W2Vmodels = self.maybe_load_wv(W2Vmodels)
        logger.info("loaded W2Vmodels, loading idf counts")
        self.idf_counts = self.maybe_load_counts(idf_counts)
        logger.info("loaded idf counts")
    ...
    def preprocess_signatures(self, load_name_counts: bool):
        ...
        for signature_id, signature in tqdm(self.signatures.items(), desc="Preprocessing signatures"):
            ...
            if self.preprocess:
                ...
                affiliations_n_grams, affiliations_word_split = get_text_ngrams_words(
                    " ".join(affiliations),
                    AFFILIATIONS_STOP_WORDS,
                )

                affiliations_ngrams_char = get_text_ngrams(" ".join(affiliations), stopwords=None, use_bigrams=True)
                ...
                signature = signature._replace(
                    ...
                    author_info_affiliations_ngrams_words=affiliations_n_grams,
                    author_info_affiliations_word_split = affiliations_word_split,
                    author_info_affiliations_ngrams_chars = affiliations_ngrams_char,
                    author_info_coauthor_ngrams_chars=get_text_ngrams(" ".join(coauthors), stopwords=None, use_bigrams=True)
                    if coauthors is not None
                    else Counter(),
                    author_info_coauthor_ngrams_words= get_text_ngrams_words(" ".join(coauthors))[0]
                    if coauthors is not None
                    else Counter(),
    ...
    def maybe_load_wv(self, W2Vmodels: Optional[Union[str, Dict]]) -> Optional[Dict]:
        """
        Load W2Vmodels for MWD 

        Returns
        -------
        dict of three W2Vmodels: title, affiliation, venue
        """
        if isinstance(W2Vmodels, str):
            if not os.path.exists(W2Vmodels+"_title.model"):
                logger.info("Create W2Vmodels")
                self.create_w2v_idf(W2Vmodels,'wv')
                logger.info("Created W2Vmodels")

            return {'title':Word2Vec.load(W2Vmodels+"_title.model"),
                    'affiliation':Word2Vec.load(W2Vmodels+"_affiliation.model"),
                    'venue':Word2Vec.load(W2Vmodels+"_venue.model"),}
        else:
            return W2Vmodels

    def maybe_load_counts(self, idf_counts: Optional[Union[str, Dict]]) -> Optional[Dict]:
        """
        Load word counts for IDF 

        Returns
        -------
        dict of three W2Vmodels: title, affiliation, venue
        """
        if isinstance(idf_counts, str):
            if not os.path.exists(idf_counts+"_title.pickle"):
                logger.info("Create idf counts")
                self.create_w2v_idf(idf_counts,'idf')
                logger.info("Created idf counts")

            with open(idf_counts+"_title.pickle", "rb") as _pickle_file:
                title = pickle.load(_pickle_file)
            with open(idf_counts+"_affiliation.pickle", "rb") as _pickle_file:
                affiliation = pickle.load(_pickle_file)
            with open(idf_counts+"_venue.pickle", "rb") as _pickle_file:
                venue = pickle.load(_pickle_file)
    ...
    def create_w2v_idf(self, parent_dir, ctype='wv'):
        class Sentences(object):
            ...
            def __iter__(self):
                if self.attribute == 'title':
                    for id in self.dataset.papers:
                        paper = self.dataset.papers[id]
                        yield paper.title_word_split

                if self.attribute == 'affiliation':
                    for id in self.dataset.signatures:
                        paper = self.dataset.signatures[id]
                        yield paper.author_info_affiliations_word_split

                if self.attribute == 'venue':
                    for id in self.dataset.papers:
                        paper = self.dataset.papers[id]
                        if paper.venue_word_split or paper.journal_word_split:
                            yield paper.venue_word_split + paper.journal_word_split

        for attr in ['title','affiliation','venue']:
            if ctype == 'wv':
                path = parent_dir+f"_{attr}.model"
                ms = Sentences(self,attr)
                model = Word2Vec(ms,min_count=1)
                model.save(path)

            if ctype == 'idf':
                path = parent_dir+f"_{attr}.pickle"
                c = Counter()
                ms = Sentences(self,attr)
                i = 0
                ncount = 0
...
def preprocess_paper_1(item: Tuple[str, Paper]) -> Tuple[str, Paper]:
    ...
    title_ngrams_words, title_word_split = get_text_ngrams_words(title)
    authors = [
        Author(
            position=author.position,
    ...
    paper = paper._replace(title=title, title_ngrams_words=title_ngrams_words,title_word_split=title_word_split,authors=authors)
    ...
    if global_preprocess:  # type: ignore
        ...
        if paper.in_signatures:
            ...
            venue_ngrams_words, venue_word_split = get_text_ngrams_words(paper.venue, stopwords=VENUE_STOP_WORDS)
            journal_ngrams = get_text_ngrams(paper.journal_name, stopwords=VENUE_STOP_WORDS, use_bigrams=True)
            journal_ngrams_words, journal_word_split = get_text_ngrams_words(paper.journal_name, stopwords=VENUE_STOP_WORDS)
            paper = paper._replace(
                title_ngrams_chars=title_ngrams_chars,
                venue_ngrams=venue_ngrams,
                venue_ngrams_words=venue_ngrams_words,
                venue_word_split=venue_word_split,
                journal_ngrams=journal_ngrams,
                journal_ngrams_words=journal_ngrams_words,
                journal_word_split=journal_word_split,
            )
...
def preprocess_paper_2(item: Tuple[str, Paper, List[MiniPaper]]) -> Tuple[str, Paper]:
    ...
    names = " ".join(authors)
    reference_details = (
        [get_text_ngrams(names.strip(), use_bigrams=True, stopwords=None), get_text_ngrams_words(names.strip())[0]],
        [get_text_ngrams(titles, use_bigrams=True), get_text_ngrams_words(titles)[0]],
        [get_text_ngrams(
            venues + " " + journals if venues != journals else venues, stopwords=VENUE_STOP_WORDS, use_bigrams=True
        ), get_text_ngrams_words( venues + " " + journals if venues != journals else venues)[0]],
        Counter(blocks),
    )
